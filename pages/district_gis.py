...
def disp_in_district_map(india_or_state, distr_kpi, value_or_change, distr_dmn):
    ...
    pcnt_scale = 1 if "Rs." in distr_kpi else 100
    table_df = pd.concat(
        [
    ...
    full_range = [
        display_df[value_or_change].min() - 0.1,
    ...
    not_reported = np.setdiff1d(
        ...
        display_df.query(f"{value_or_change}.notna()")["District name"].values,
    )
    not_reported_geo = [
    ...
    display_df = (
                ...
                pd.DataFrame(
                    {
                        ...
                        else "Note_Change": ["Value NOT Reported: (-1000)"]
                        * len(not_reported),
                    }
                ),
            ],
        ...
        .drop_duplicates(subset=["District_geo", "District_name"], ignore_index=True)
        .fillna({value_or_change: -1000})
    )
    ...
    cmap_fig = px.choropleth(
        ...
        color_continuous_scale=nan_blue_y_red,
    ...
    num_format = (
        FormatTemplate.money(0) if "Rs." in distr_kpi else FormatTemplate.percentage(0)
    )
    table_col_format = [
        {"name": i, "id": i}
